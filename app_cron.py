...
from fpdf.enums import XPos, YPos # 導入新版 FPDF 座標定位
...
# ==========================================
# 3. 圖表生成 (修復死結與移除 Kaleido 警告)
# ==========================================
def generate_charts(ticker):
    print(f">>> 步驟 2: 正在生成 {ticker} 的圖表...")
    try:
        # threads=False 避免在 GitHub Runner 上死結
        ...
        last_close = float(df_daily['Close'].values[-1])
        last_ma200 = float(df_daily['200MA'].values[-1])
        ...
        # 移除 engine='kaleido' 以消除警告，這是最新的標準寫法
...
def get_ai_insight(row, is_above_200):
    ...
    for attempt in range(3):
        try:
            ...
            # 強制冷卻 45 秒，這對於免費版 Pro 帳號是絕對必要的
            time.sleep(45) 
            return response.text
        except Exception as e:
            if "429" in str(e) or "503" in str(e):
                print(f"⚠️ AI 繁忙 ({row['Ticker']})，嘗試第 {attempt+1} 次重試...")
                time.sleep(60) # 遇到頻率限制，停一分鐘
            else:
                return f"AI 錯誤: {e}"
    return "跳過分析 (配額耗盡)"

# ==========================================
# 5. PDF 報告 (徹底修復 DeprecationWarning)
# ==========================================
class PDFReport(FPDF):
    def header(self):
        self.set_font('helvetica', 'B', 16)
        # 修正：使用 new_x=XPos.LMARGIN, new_y=YPos.NEXT 取代 ln=True
        self.cell(0, 10, text=f'US Stock AI Report - {datetime.date.today()}', align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.ln(10)

def create_report(df):
    print(">>> 步驟 3: 正在製作 PDF 報告...")
    pdf = PDFReport()
