# 版本號碼：v1.5.1
print(">>> [系統啟動] 正在執行 v1.5.1 正式營運版：解封數量限制，開啟全量 AI 技術診斷...")
...
def fetch_and_filter_stocks():
    ...
    try:
        ...
        # 解封限制：TEST_MODE 下抓 2 支，正式環境抓取全量
...
def generate_stock_images(ticker):
    try:
        ...
        img_max = generate_chart(df_all.tail(min(len(df_all), 756))) # 動態 MAX 邏輯

        # 1分鐘圖：整合盤後、時區校準與 Top 10 爆量
        ...
        if not df_1m.empty:
            ...
            for idx, row in spikes.sort_values(by='Volume', ascending=False).head(10).iterrows():
                ...
                fig2.add_annotation(x=idx, y=row['High'], text=symbol, showarrow=True, arrowhead=1, arrowcolor=t_color, font=dict(size=11, color=t_color, weight='bold'), bgcolor="black", opacity=0.9, ay=-40, yshift=5)
...
def get_batch_ai_insights(df_subset):
    ...
    if TEST_MODE: return {t: f"<p style='color:#666;'>[測試] 正在診斷 {t}...</p>" for t in tickers}
    if not GEMINI_KEY: return {t: "❌ 缺少 API KEY" for t in tickers}

    summary = "".join([f"- {r['Ticker']}: ${r['Price']} ({r['Change']}%)\n" for _, r in df_subset.iterrows()])
    prompt = f"分析美股異動股技術面：\n{summary}\n請結合圖中 MACD/RSI/1m爆量給予100字內精煉建議。繁體中文。回傳 JSON：{{\"Ticker\": \"分析內容\"}}"
    try:
        ...
        time.sleep(40); return json.loads(raw_text)
    except: return {t: "⚠️ 分析暫時無法產出" for t in tickers}
...
def create_html_report(df):
    ...
    print(f">>> [步驟 2] 開始 AI 全量診斷 (共 {len(df)} 支)...")
    all_insights = {}
    for i in range(0, len(df), 2):
        all_insights.update(get_batch_ai_insights(df.iloc[i:i+2]))

    ICON_URL = "https://cdn-icons-png.flaticon.com/512/2422/2422796.png"
    html_header = f"""<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"><link rel="icon" href="{ICON_URL}"><link rel="apple-touch-icon" href="{ICON_URL}"><title>AI 美股深度掃描</title>
    <style>
    ...
    <script>
        ...
        async function shareTicker(t, p) {{
            const s = {{ title: `📈 AI 掃描: ${{t}}`, text: `代碼 ${{t}} 目前 $${{p}}。點擊查看詳細 AI 技術分析報告。`, url: window.location.origin + window.location.pathname + '?ticker=' + t }};
    ...
    </script></head><body><div class="container" id="top"><div class="history-bar"><div style="font-weight:bold;margin-right:10px;color:#003366;white-space:nowrap;">📅 歷史存檔：</div>{history_links}</div>
    ...
    for _, row in df.iterrows():
        ...
        ins = all_insights.get(row['Ticker'], "分析產出中...")
        if i1:
            ...
            <div class="analysis-box"><strong>🛡️ AI 策略師診斷：</strong><br>{ins}<div class="btn-group"><button class="action-btn share-btn" onclick="shareTicker('{row['Ticker']}', '{row['Price']}')">📲 分享此股票</button><a href="#top" class="action-btn">⬆ 返回總表</a></div></div></div>"""
    ...
    print(f"✅ v1.5.1 正式版產出完成 (共分析 {len(df)} 支股票)。")
