# 版本號碼：v1.3.3
print(">>> [系統啟動] 正在執行 v1.3.3：指標全回歸 + iPhone 原生分享功能...")
...
def get_batch_ai_insights(df_subset):
    ...
    if TEST_MODE: return {t: f"<p style='color:#666;'>[測試] 批量分析中 - {t}</p>" for t in tickers}
    data_summary = "".join([f"- {r['Ticker']}: ${r['Price']} ({r['Change']}%)\n" for _, r in df_subset.iterrows()])
    prompt = f"請專業分析以下美股技術走勢，結合 MACD/RSI14 提供 100 字內繁體中文建議。必須回傳 JSON 格式：{{\"Ticker\": \"分析內容\"}} \n數據如下：\n{data_summary}"
    try:
        client = genai.Client(api_key=GEMINI_KEY)
        response = client.models.generate_content(model=TARGET_MODEL, contents=prompt)
        raw_text = response.text.strip().replace('```json', '').replace('```', '')
        insights = json.loads(raw_text)
        time.sleep(35); return insights
    except: return {t: "⚠️ 暫時無法產出分析" for t in tickers}

# ==========================================
# 5. HTML 生成 (加入分享按鈕與原生分享腳本)
# ==========================================
def create_html_report(df):
    ...
    <html lang="zh-TW"><head>
        ...
        <script>
            // Deep Linking 與 原生分享邏輯
            async function shareTicker(ticker, price) {{
                const shareData = {{
                    title: `📈 AI 掃描建議: ${{ticker}}`,
                    text: `發現 ${{ticker}} 異動！目前價格 $${{price}}。點擊查看詳細 AI 技術分析報告。`,
                    url: window.location.origin + window.location.pathname + '?ticker=' + ticker
                }};
                try {{
                    if (navigator.share) {{ await navigator.share(shareData); }} 
                    else {{ alert('網址已複製：' + shareData.url); navigator.clipboard.writeText(shareData.url); }}
            ...
            window.onload = function() {{
                ...
                if (ticker) {{
                    const element = document.getElementById(ticker.toUpperCase());
                    if (element) {{ setTimeout(() => {{ element.scrollIntoView({{ behavior: 'smooth', block: 'start' }}); }}, 600); }}
                }}
            }};
        </script>
    </head>
        ...
        if img1:
            ...
            <div class="stock-card" id="{row['Ticker']}">
                ...
                <div class="analysis-box">
                    <strong>🛡️ AI 策略師診斷：</strong><br>{insight}
                    <div class="btn-group">
                        <button class="action-btn share-btn" onclick="shareTicker('{row['Ticker']}', '{row['Price']}')">📲 分享此股票</button>
                        <a href="#top" class="action-btn">⬆ 返回總表</a>
                    </div>
                </div>
            </div>"""
