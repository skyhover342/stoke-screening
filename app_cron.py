# 版本號碼：v1.4.4
print(">>> [系統啟動] 正在執行 v1.4.4：同步 1m 與日線成交量顏色，強化視覺一致性...")
...
def generate_stock_images(ticker):
    try:
        ...
        if not df_1m.empty:
            ...
            # --- 強化成交量顯色：同步 rgba(210, 210, 210, 0.8) 並移除邊框 ---
...
# 剩餘邏輯 (get_batch_ai_insights, create_html_report) 維持 v1.4.3 穩定內容
# ... (此處省略重複內容以加速回覆，但您的代碼應保留完整版本)
...
def create_html_report(df):
    ...
    for _, row in df.iterrows():
        ...
        if img_1y:
            cards += f"""<div class="stock-card" id="{row['Ticker']}"><div class="card-header-row"><div>{row['Ticker']}</div><div>{row['Industry']}</div><div>{row['MarketCap']}</div><div>{row['PE']}</div><div>${row['Price']}</div><div style="color:#ffcccc;">+{row['Change']}%</div><div>{row['Volume']}</div></div><div class="toggle-bar"><button id="btn-1y-{row['Ticker']}" class="toggle-btn active" onclick="switchPeriod('{row['Ticker']}', '1y')">📅 1Y 日線</button><button id="btn-max-{row['Ticker']}" class="toggle-btn" onclick="switchPeriod('{row['Ticker']}', 'max')">♾️ MAX (Up to 3Y)</button></div><div class="chart-stack"><img id="img-1y-{row['Ticker']}" src="data:image/png;base64,{img_1y}"><img id="img-max-{row['Ticker']}" src="data:image/png;base64,{img_max}" style="display:none;"><img src="data:image/png;base64,{img_1m}"></div><div class="analysis-box"><strong>🛡️ AI 策略師診斷：</strong><br>{insight}<div class="btn-group"><button class="action-btn share-btn" onclick="shareTicker('{row['Ticker']}', '{row['Price']}')">📲 分享此股票</button><a href="#top" class="action-btn">⬆ 返回總表</a></div></div></div>"""
