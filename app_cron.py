# 版本號碼：v1.2.3
print(">>> [系統啟動] 正在執行 v1.2.3：加入 Favicon 與 iPhone 桌面圖標優化...")
...
def generate_stock_images(ticker):
    try:
        ...
        fig1.add_trace(go.Bar(x=df_1y.index, y=df_1y['Hist'], marker_color=['lime' if v>=0 else 'red' for v in df_1y['Hist']], name="MACD"), row=2, col=1)
...
def create_html_report(df):
    ...
    <html lang="zh-TW"><head>
        ...
        <meta name="apple-mobile-web-app-title" content="AI 美股掃描">
        ...
        <style>
            ...
            .summary-table {{ width: 100%; border-collapse: collapse; background: white; margin-bottom: 40px; font-size: 12px; }}
    ...
    <body><div class="container" id="top">
        <div class="history-bar"><div style="font-weight:bold;margin-right:15px;color:#003366;">📅 歷史報告：</div>{history_links}</div>
    ...
    for _, row in df.iterrows():
        ...
        if img1:
            ...
            <div class="stock-card" id="{row['Ticker']}">
                ...
                <div class="analysis-box"><strong>🛡️ AI 策略師診斷：</strong><br>結合 MACD/RSI 的 AI 分析 (待執行)<a href="#top" class="back-btn">⬆ 返回總表</a><div style="clear:both;"></div></div>
            </div>"""
